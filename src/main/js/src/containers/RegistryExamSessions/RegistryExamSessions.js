...
class RegistryExamSessions extends PureComponent {
  ...
  getOrganizationFromRegistry = () => {
    if (
      !this.state.registryItem &&
      !this.state.organizationNotFound &&
      this.props.orgRegistry.length > 0
    ) {
  ...
  componentDidMount = () => {
    this.props.match.params.oid &&
      this.props.onFetchExamSessions(this.props.match.params.oid);
  ...
  componentDidUpdate = (prevProps, prevState) => {
    if (!prevState.selectedSession && this.state.selectedSession) {
      this.props.onFetchSessionParticipants(
        this.props.match.params.oid,
  ...
  selectSessionHandler = session => this.setState({ selectedSession: session });
  ...
  render() {
    ...
    const getLocationName = () => {
      ...
      return this.state.selectedSession.location &&
        this.state.selectedSession.location[0]
        ...
        {this.state.selectedSession && (
          <>
            ...
            <h2>
              ...
              {moment(this.state.selectedSession.session_date).format(
                DATE_FORMAT,
              )}
            </h2>
          </>
      ...
      return (
        <>
          <h1>
            {organization &&
              organization.nimi &&
              registryUtil.getLocalizedName(organization.nimi)}
          </h1>
          <h3>
...
const mapStateToProps = state => {
  return {
    ...
    error: state.registryDetails.error || state.exam.error,
  };
};
...
export default connect(
  mapStateToProps,
  mapDispatchToProps,
)(withTranslation()(withErrorHandler(RegistryExamSessions)));
