describe('Exam sessions', () => {
  ...
  const getLanguageDataId = (dateId, lang, level) => `[data-cy=exam-dates-row-language-${dateId}-${lang}-${level}]`
  const getNewLanguageDataId = (lang, level) => getLanguageDataId(newDateId, lang, level);
  const getModifiedLanguageDataId = (lang, level) => getLanguageDataId(modifyDateId, lang, level);
  ...
  const chooseFlatpickerDate = (day, month, year) => {
    ...
    cy.get('.open .flatpickr-day').contains(new RegExp(`^${day}$`)).click()
  }
  ...
  it('new exam date can be created', () => {
    ...
    cy.get('[data-cy=exam-dates-table-rows]').contains(new RegExp(`^${'1.3.2050'}$`))
  ...
  it('can enable post admissions for exam date', () => {
    ...
    cy.get('[data-cy=exam-dates-table-rows]')
      ...
      .should('have.length', originalRowCount);
    cy.get('[data-cy=exam-dates-table-rows]').contains('30.1.2081');
    cy.get(`[data-cy=exam-dates-list-post-admission-${modifyDateId}`).contains('15.12.2080 - 15.1.2081');
  });

  it('can delete the exam date', () => {
    cy.get(`[data-cy=exam-dates-list-checkbox-${modifyDateId}]`)
    ...
    cy.get('[data-cy=exam-dates-table-rows]')
      .find('label')
      .should('have.length', originalRowCount - 1);
    cy.get(`[data-cy=exam-dates-list-date-${modifyDateId}]`).should('not.exist');
  });
